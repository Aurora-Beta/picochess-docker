...
ENTRY_STRUCT = struct.Struct(">QHBBI")
...
class MemoryMappedReader:
    ...
    def __getitem__(self, index: int) -> Entry:
        ...
        try:
            key, raw_move, weight, learn, n = ENTRY_STRUCT.unpack_from(self.mmap, index * ENTRY_STRUCT.size)
        except struct.error:
            raise IndexError()
        ...
        return Entry(key, raw_move, weight, learn, n, move)
    ...
    def bisect_key_left(self, key: int) -> int:
        ...
        while lo < hi:
            mid = (lo + hi) // 2
            mid_key, _, _, _, _ = ENTRY_STRUCT.unpack_from(self.mmap, mid * ENTRY_STRUCT.size)
            if mid_key < key:
                lo = mid + 1
            else:
                hi = mid
    ...
    def find_all(self, board: Union[chess.Board, int], *, minimum_weight: int = 1, exclude_moves: Container[chess.Move] = []) -> Iterator[Entry]:
        ...
        while i < size:
            ...
            if context:
                ...
                entry = Entry(entry.key, entry.raw_move, entry.weight, entry.learn, entry.n, move)
